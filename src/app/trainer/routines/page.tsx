...
export default function TrainerRoutinesPage() {
  ...
  const [nutritionItems, setNutritionItems] = useState<Meal[]>([
    { id: undefined, nutrition_plan: undefined, meal_type: "breakfast", time: "", description: "" },
  ]);
  ...
  useEffect(() => {
    const fetchRoutines = async () => {
      ...
      try {
        ...
        if (!Array.isArray(rawData)) { console.error("API did not return an array:", rawData); return; }
        const mappedRoutines: RoutinePlan[] = rawData.map((routine) => ({
          ...
          frequency: routine.frequency, exercises: routine.exercises, client: routine.client,
          trainer: routine.trainer, nutrition: routine.nutrition,
        }));
  ...
  const handleSaveRoutine = async () => {
    ...
    setIsSaving(true);
    const routineData = {
        clientId: selectedClient, routineName, startDate, frequency,
        exercises: exercises.map(ex => ({ name: ex.name, sets: ex.sets, reps: ex.reps, unit: ex.unit, notes: ex.notes || "" })),
    };
    try {
        const token = localStorage.getItem('accessToken');
        ...
        toast({ title: t('toastRoutineSaved'), description: t('toastRoutineSavedDesc', { routineName, clientName: clients.find(c => c.id === selectedClient.toString())?.name || 'Client' }) });
    } catch (err) {
  ...
  const handleAddMeal = () => {
    setNutritionItems((prev) => [ ...prev, { id: undefined, nutrition_plan: undefined, meal_type: "breakfast", time: "", description: "" }]);
  };
  ...
  const handleChange = (index: number, key: keyof Meal, value: string) => {
    setNutritionItems((prev) => { const updated = [...prev]; updated[index] = { ...updated[index], [key]: value }; return updated; });
  };
  ...
  const fetchClientPlan = async (clientId: number) => {
    ...
    } catch (error) { console.error("Error fetching client plans:", error); return null; }
  };

  const handleSavePlan = async () => {
    setIsSaving(true);
    if (!selectedClient) { // Ensure client is selected for nutrition plan
        toast({ title: t('toastMissingInfo'), description: t('toastMissingInfoDesc'), variant: "destructive"});
        setIsSaving(false);
        return;
    }
    ...
    const payload: NutritionPlan = {
      plan: clientPlan.planId, notes: "",
      meals: nutritionItems.map((meal) => ({ meal_type: meal.meal_type, time: meal.time, description: meal.description })),
    };
    try {
      const token = localStorage.getItem("accessToken");
      ...
      if (!res.ok) {
        ...
        toast({ title: t('toastErrorSavingNutrition'), description: err?.detail || JSON.stringify(err), variant: "destructive"});
      } else {
        const savedPlan: NutritionPlan = await res.json();
        toast({ title: t('toastNutritionSaved'), description: t('toastNutritionSavedDesc') });
        setNutritionItems(savedPlan.meals.map((m) => ({ ...m }))); // simplified mapping
      }
    } catch (error) {
      toast({ title: t('toastNetworkError'), description: t('toastNetworkErrorDesc'), variant: "destructive" });
    }
    setIsSaving(false);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-primary"><ClipboardEdit className="h-6 w-6" /> {t('newRoutineTitle')}</CardTitle>
                ...
                <Label htmlFor="frequency-select">{t('frequencyLabel')}</Label>
                 <Select value={frequency} onValueChange={(val: 'daily' | 'weekly' | 'custom') => setFrequency(val)} disabled={isSaving}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="custom">{t('frequencyCustom')}</SelectItem>
                    </SelectContent>
                </Select>
            </div>
          </div>
        ...
        <CardFooter><Button onClick={handleSaveRoutine} disabled={isSaving} className="w-full md:w-auto"><Save className="mr-2 h-4 w-4" />{isSaving ? t('savingRoutineButton') : t('saveRoutineButton')}</Button></CardFooter></Card>
      <Card className="shadow-sm">
        ...
        <CardContent>{routines.length === 0 ? (<p className="text-muted-foreground text-center py-8">{t('assignedRoutinesListDescription')}</p>
          ) : (<div className="space-y-4">{routines.map((routine) => (
                <Card key={routine.planId} className="border p-4"><p className="text-sm text-muted-foreground">Start Date: {routine.startDate}</p><p className="text-sm">Frequency: {routine.frequency}</p>
                   <ul className="list-disc pl-5 mt-2">{routine.exercises?.map((ex, index) => (<li key={`${ex.name}-${index}`}>{ex.name} — {ex.sets} sets × {ex.reps} {ex.unit}</li>))}</ul>
                  <div className="mt-4 flex gap-2"><Button variant="outline" onClick={() => handleEditRoutine(routine)}>{t('editButton')}</Button><Button variant="destructive" onClick={() => handleDeleteRoutine(routine.planId)}>{t('deleteButton')}</Button></div></Card>))}</div>)}</CardContent></Card>
      <Card className="shadow-lg">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select value={selectedClient !== null ? String(selectedClient) : undefined} onValueChange={(val) => setSelectedClient(Number(val))} disabled={isSaving}>
                  ...
                  <SelectContent>{clients.map((client) => (<SelectItem key={client.id} value={String(client.id)}><div className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground" />{client.name}</div></SelectItem>))}</SelectContent></Select></div>
            <div className="space-y-2">
              <Label htmlFor="start-date-nutrition" className="flex items-center gap-1"><CalendarDays className="h-4 w-4" />{t('startDateLabel')}</Label>
              <Input id="start-date-nutrition" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} disabled={isSaving}/></div></div>
          <div className="space-y-4 pt-4 border-t">
            <div className="flex justify-between items-center">
              ...
              <Card key={index} className="p-4 bg-secondary/30 relative shadow-sm border">
                ...
                <div className="grid md:grid-cols-3 gap-4 items-end">
                  <div className="space-y-1">
                    ...
                    <select id={`meal-type-${index}`} value={item.meal_type} onChange={(e) => handleChange(index, "meal_type", e.target.value as "breakfast" | "lunch" | "dinner")} className="w-full border rounded px-2 py-1.5 text-sm h-10" disabled={isSaving}>
                      <option value="breakfast">{t('mealTypeBreakfast')}</option><option value="lunch">{t('mealTypeLunch')}</option><option value="dinner">{t('mealTypeDinner')}</option></select></div>
                  <div className="space-y-1">
                    <Label htmlFor={`meal-time-${index}`}>{t('mealTimeLabel')}</Label>
                    <Input id={`meal-time-${index}`} type="time" value={item.time} onChange={(e) => handleChange(index, "time", e.target.value)} disabled={isSaving} required /></div>
                  <div className="space-y-1 md:col-span-1">
                    <Label htmlFor={`meal-desc-${index}`}>{t('mealDetailsLabel')}</Label>
                    <Textarea id={`meal-desc-${index}`} placeholder={t('mealDetailsPlaceholder')} rows={2} value={item.description} onChange={(e) => handleChange(index, "description", e.target.value)} disabled={isSaving} required /></div></div></Card>))}</div>
        </CardContent>
        <CardFooter><Button onClick={handleSavePlan} disabled={isSaving} className="w-full md:w-auto"><Save className="mr-2 h-4 w-4" />{isSaving ? t('savingNutritionPlanButton') : t('saveNutritionPlanButton')}</Button></CardFooter></Card></div>
  );
}
