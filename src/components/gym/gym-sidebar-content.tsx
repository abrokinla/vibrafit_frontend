// src/components/gym/gym-sidebar-content.tsx
'use client';

import { Link, useRouter, usePathname } from '@/navigation'; 
import { LayoutDashboard, Users, UserCheck, Settings, LogOut, Upload } from 'lucide-react';
...
interface GymSidebarData { 
  name: string;
  profilePictureUrl: string | null;
}

export default function GymSidebarContent() {
  const t = useTranslations('GymSidebar');
  const pathname = usePathname();
  const router = useRouter();
  const { toast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [gymData, setGymData] = useState<GymSidebarData | null>(null); 
  const [isUploading, setIsUploading] = useState(false);

  const navItems = [
    { href: '/gym/dashboard', label: t('dashboard'), icon: LayoutDashboard },
    { href: '/gym/clients', label: t('clients'), icon: Users },
    { href: '/gym/coaches', label: t('coaches'), icon: UserCheck },
    { href: '/gym/settings', label: t('settings'), icon: Settings },
  ];
    ...
    const load = async () => {
      try {
        ...
        if (err.message === 'NO_CREDENTIALS' || err.message === 'UNAUTHORIZED') {
          ...
          console.error('Failed to load sidebar gym data:', err);
        }
      }
    };
    load();
    ...
    if (file) {
      ...
      try {
        ...
        if (result.success && result.newUrl) {
          setGymData(prev => prev ? { ...prev, profilePictureUrl: result.newUrl } : null);
          toast({
      ...
      <SidebarHeader className="p-4 group-data-[state=expanded]/sidebar:border-b">
        <div className="flex flex-col items-center space-y-2 group-data-[state=collapsed]/sidebar:hidden">
          <Avatar
            ...
            tabIndex={0}
          >
            <AvatarImage src={gymData?.profilePictureUrl || undefined} alt={gymData?.name || 'Gym Logo'} />
            <AvatarFallback className="text-3xl bg-muted">
              {isUploading ? <Upload className="h-8 w-8 animate-pulse text-primary" /> : (gymData?.name ? gymData.name.charAt(0) : 'G')}
            </AvatarFallback>
          </Avatar>
          <input
            type="file"
          ...
          />
          {gymData?.name && <p className="text-sm font-medium">{gymData.name}</p>}
              ...
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <AvatarFallback className="text-xl bg-muted">
                    {isUploading ? <Upload className="h-5 w-5 animate-pulse text-primary" /> : (gymData?.name ? gymData.name.charAt(0) : 'G')}
                  </AvatarFallback>
                </Avatar>
              </TooltipTrigger>
              <TooltipContent side="right" align="center">
                <p>{gymData?.name || t('gymProfileTooltip')}</p>
